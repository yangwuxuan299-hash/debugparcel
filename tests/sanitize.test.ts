...
test("percent-encoded copies of discovered values fail the final audit", () => {
  for (const encoded of ["tenant%2Dsec%72et", "tenant%252Dsecret", "%2574enant-secret"]) {
    const result = scanDiagnostics({
      harText: harWithEntry({
        request: {
          method: "GET",
          url: "https://api.example.com/private",
          cookies: [{ name: "auth_cookie", value: "tenant-secret" }],
        },
        response: { status: 200, content: {} },
      }),
      consoleText: JSON.stringify({ message: `copy=${encoded}` }),
    });

    assert.equal(result.auditPassed, false, encoded);
    assert.match(result.auditMessage, /cookie value survived/i);
  }
});
...
test("custom rules redact mixed URI and form-encoded variants", () => {
  const result = scanDiagnostics({
    consoleText: "tenant%2Dsec%72et tenant%2dsecret tenant%252Dsecret Alice%20Smith Alice+Smith Alice%252BSmith",
    customTerms: ["tenant-secret", "Alice Smith"],
  });

  assert.equal(result.auditPassed, true);
  assert.doesNotMatch(result.sanitizedConsole ?? "", /tenant%25?2[dD]|Alice(?:%25?20|%252[Bb]|\+)Smith/);
  assert.match(result.sanitizedConsole ?? "", /\[SECRET_\d+\]/);
});

test("custom rules preserve aliases generated by automatic sanitization", () => {
  const result = scanDiagnostics({
    consoleText: 'password="supersecret"',
    customTerms: ["SECRET"],
  });

  assert.equal(result.auditPassed, true);
  assert.match(result.sanitizedConsole ?? "", /password="\[SECRET_\d+\]"/);
  assert.doesNotMatch(result.sanitizedConsole ?? "", /\[\[SECRET_/);
  assert.equal(result.findings.some((finding) => finding.raw === "SECRET"), false);
});

test("custom rules preserve URI-encoded aliases generated by automatic sanitization", () => {
  const result = scanDiagnostics({
    consoleText: "GET /?q=%5BQUERY_1%5D",
    customTerms: ["QUERY"],
  });

  assert.equal(result.auditPassed, true);
  assert.match(result.sanitizedConsole ?? "", /%5BQUERY_\d+%5D/);
  assert.equal(result.findings.some((finding) => finding.raw === "QUERY"), false);
});

test("custom rules still redact alias-shaped source text", () => {
  const result = scanDiagnostics({
    consoleText: "note=[SECRET_1]",
    customTerms: ["[SECRET_1]"],
  });

  assert.equal(result.auditPassed, true);
  assert.equal(result.sanitizedConsole?.includes("[SECRET_1]"), false);
  assert.match(result.sanitizedConsole ?? "", /\[SECRET_2\]/);
});
...
test("HAR-shaped console JSON is rejected before bodies can be exported", () => {
  const entry = {
    request: {
      method: "POST",
      url: "https://api.example.com/private",
      postData: { text: "BluePiano77" },
    },
    response: {
      status: 500,
      content: { text: "CrimsonOrchid88" },
    },
  };
  const disguisedHarFiles = [
    harWithEntry(entry),
    JSON.stringify({ log: { entries: [entry] } }),
    JSON.stringify({ log: { version: 1.2, entries: [entry] } }),
  ];

  for (const disguisedHar of disguisedHarFiles) {
    assert.throws(
      () => scanDiagnostics({ consoleText: disguisedHar }),
      (error: unknown) => {
        assert.ok(error instanceof Error);
        assert.match(error.message, /looks like a HAR archive/i);
        assert.doesNotMatch(error.message, /BluePiano77|CrimsonOrchid88/);
        return true;
      },
    );
  }
...
test("invalid HAR structures fail closed while an empty HAR remains valid", () => {
  assert.throws(
    () => scanDiagnostics({ harText: JSON.stringify({ log: { version: "1.2", entries: {} } }) }),
    /not a valid HAR document/i,
  );
  assert.throws(
    () => scanDiagnostics({
      harText: JSON.stringify({
        log: { version: "1.2", entries: [{ request: {}, response: null }] },
      }),
    }),
    /invalid HAR entry/i,
  );

  const empty = scanDiagnostics({
    harText: JSON.stringify({ log: { version: "1.2", entries: [] } }),
  });
  assert.equal(empty.auditPassed, true);
  assert.equal(empty.omittedBodies, 0);
});
